...
- **スコープ別テスト実行**:
  ...
  - **画面・UI変更時の E2E 検証 (`npm run test:e2e`)**: UI コンポーネント、ルーティング、フォーム、文言を変更した場合は、必ず Playwright E2E テストを実行して検証する（対象スペック個別指定可: `npx playwright test test/e2e/guide.spec.ts`）。
...
- **品質チェックの 100% PASS**: `npm run check`（Linter, 型チェック, ビルド）の全パスを確認する。
