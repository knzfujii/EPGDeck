...
- **フェーズ別の二段階検証プロトコル**:
  - **1. 実装・試行錯誤フェーズ（超高速フィードバック）**:
    - コード修正中は全体テストを待たず、型チェック＋単体テスト `npm run check:quick`（約 2〜3 秒）や対象画面の E2E スペック単体 `npm run test:e2e:file -- <spec-path>`（約 1.5〜2 秒）で高速反復する。
  - **2. 完了報告・コミット承認要請前（DoD / 厳格な全件検証・義務）**:
    - 他画面や共通コンポーネントへの意図しない波及（リグレッション事故）を 100% 排除するため、**作業完了報告やコミット承認要請の前には必ず Playwright E2E 全件を含む `npm run check` を実行し、全件 PASS を確認すること**。
    - **重大な規律違反（絶対厳禁）**: プロダクションコード変更を伴うタスクで全件検証を通過しないまま完了報告やコミット承認要請を行うことは、品質保証上の重大な怠慢・規律違反とする。
- **フロントエンド・UI 設計規約**:
...
- **総合品質チェック 100% PASS (DoD)**: プロダクションコード変更を伴うコミットでは必ず `npm run check` の全パスを確認し、エビデンス（通過件数・エラー 0）を提示する。
- **ドキュメント・非コード変更時のテスト免除規約**: `docs/**`, `*.md`, `.gitignore`, `LICENSE` 等、実行コードやビルド成果物に影響しないファイルのみの変更時は、E2E 実行をスキップし、フォーマッタ確認および差分レビューのみでコミット承認要請を行ってよい。
