...
-   **超高速・省メモリな Web インターフェイス (Svelte 5 + Tailwind CSS v4)**
    ...
    -   **自動録画ルール管理 (`/rule`)**: 実予約数の集計バッジ表示、EPGStation 完全互換の詳細ルール作成・編集ページ（`/rule/edit`、保存先ストレージ/サブディレクトリ/エンコード3系統）。
    -   **予約一覧 (`/reserves`)**: ワンクリックでの手動予約キャンセル（取消）＆ ルール予約スキップ / スキップ解除（復活）。
    -   **リアルタイムシステムログ (`/logs`)**: log4js による統合ログを Web 画面上でリアルタイムストリーミング追尾（`tail -f` 相当）、レベル・プロセス別フィルタ、キーワード検索、ログ保存。
...
-   Linux / macOS
-   [Node.js](http://nodejs.org/) : `^22.3.0 ~ v26.x` (推奨: `v26.x LTS`、リポジトリ内に `.mise.toml` を同梱しており、[mise](https://mise.jdx.dev/) に対応しています)
