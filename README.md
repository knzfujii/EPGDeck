...
### 録画ファイル移動・DB更新ツール
特定のキーワードに合致する録画動画ファイルを別のストレージ（親ディレクトリ）やサブディレクトリへ安全に移動し、データベース（`video_file`）のパス情報を一括更新します。

- **高速 ＆ 可視化**: 同一ドライブ間は一瞬でアトミックに移動（`rename`）、異ドライブ間は大容量転送時のリアルタイム進捗バー（％、転送量）を表示。
- **安全な空フォルダ掃除**: 移動完了後に空になった親フォルダを安全に自動削除（`--keep-empty-dirs` で保持も可能、ルートは絶対保護）。
- **トランザクション保護**: 万が一 DB 更新に失敗した場合は移動先から元へファイルを自動復元（ロールバック）。

```bash
# 対話形式で実行（検索キーワード、移動元・移動先ディレクトリ、dry-run などを順に入力）
npm run move-recorded

# 引数を指定してシミュレーション（dry-run）
npm run move-recorded -- "検索キーワード" --src 1 --dst 2 --dest-relpath "anime" --dry-run

# 空フォルダを残して移動したい場合
npm run move-recorded -- "検索キーワード" --src 1 --dst 2 --keep-empty-dirs

# オプション一覧の確認
npm run move-recorded -- --help
```
