...
-   **超高速・省メモリな Web インターフェイス (Svelte 5 + Tailwind CSS v4)**
    ...
    -   **録画一覧 (`/recorded`)**: **15,000件超の録画アーカイブ対応**（年・月セレクターによる瞬間ジャンプ、ジャンルチップ絞り込み、テーブル/カード切り替え、保護トグル）。
    -   **予約一覧 (`/reserves`)**: ワンクリックでの手動予約キャンセル（取消）＆ ルール予約スキップ / スキップ解除（復活）。
    -   **ルール一覧 (`/rule`)**: 実予約数の集計バッジ表示、EPGStation 完全互換の詳細ルール作成・編集ページ（`/rule/edit`、保存先ストレージ/サブディレクトリ/エンコード3系統）。
