<script lang="ts">
    ...
    import { Film, CheckCircle2, Trash2 } from '@lucide/svelte';

    let running = $state<any[]>([]);
    let waitList = $state<any[]>([]);
    let isLoading = $state(true);

    async function fetchEncode() {
        isLoading = true;
        try {
            ...
            snackbar.open({ text: 'エンコード情報の取得に失敗しました', color: 'error' });
        } finally {
            isLoading = false;
        }
    }
...
<div class="space-y-5 w-full max-w-full min-w-0">
    <div class="rounded-2xl border border-slate-200 bg-white p-4 sm:p-5 shadow-xs dark:border-slate-800 dark:bg-slate-900">
        ...
        <p class="text-xs text-slate-500 dark:text-slate-400">録画ファイルのバックグラウンド変換キュー</p>
    </div>
        ...
        {:else}
            <div class="mt-3 space-y-3">
                {#each running as item}
                    <div class="flex items-center justify-between rounded-xl border border-slate-100 bg-slate-50/50 p-4 dark:border-slate-800 dark:bg-slate-800/40">
                        <div>
