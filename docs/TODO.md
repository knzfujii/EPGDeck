...
- [ ] **ルール一覧のソート・並び替え機能 (`Rule.svelte`)**
  - 優先度（Priority）順（高い順/低い順）、登録順（新しい順/古い順）、キーワード順（昇順/降順）、予約件数順（多い順）、有効/無効優先など、柔軟な並び替え UI と状態保持
- [ ] **二重録画防止履歴の可視化・管理機能 (`recorded_history`) と個別再録画許可**
  - ルールの二重録画防止（`avoidDuplicate`）で保持されている履歴データの検索・閲覧画面の実装
  - 特定番組の履歴個別削除（再録画の許可）API / UI の提供
  - **削除時の自動予約再評価連動**: 手動更新ボタンを画面に置かず、履歴削除 API の内部で自動的に予約・ルールの再評価（`updateAll`）を即座にトリガーし、ユーザーに手動操作を強いることなく即時に再予約枠を生成する設計とする
