...
- [ ] **録画 MP4 直接再生時のシーク遅延 (Chrome tx3g バグ) 対策と VTT 別保存化の検討**
  - 現在、MP4 エンコード時に字幕を内部トラック (`tx3g` / `mov_text`) として埋め込んでいるが、Chrome 等のブラウザ仕様により「次の字幕パケットが見つかるまで数十秒間、動画の受信をスロットリングして再生をポーズする」という致命的なシーク遅延バグ（18〜27秒待ち）が発生している。
...
| **Playwright E2E テスト包括的強化（番組表・検索・サムネイル再作成・リードオンリーモード）** | 番組表からの録画予約作成・解除・ルール検索/ルール編集遷移、検索画面からの条件引き継ぎルール作成導線（スマートキーワード抽出）、録画詳細画面でのサムネイル再作成モーダル（秒数指定リクエスト送信）、リードオンリーモード認証・ロック解除/再ロック（`readOnlyStore.unlock` の失敗例外スロー改善含む）の E2E シナリオ（`readonly.spec.ts` 新設含む）を配備（計46件全件 PASS） | [テスト仕様書](dev/testing.md#3-playwright-e2e-テスト基盤)、[画面変更仕様書](dev/epgdeck_change_spec.md#5-リードオンリーモード閲覧専用モード仕様) |
| **Playwright E2E テスト強化 第2弾（ダッシュボード・予約一覧・オンエア画面）** | ダッシュボード（録画中カード・進行バー・3択停止モーダル・予約警告アコーディオン）、予約一覧（すべて/競合/スキップ/重複タブ絞り込み・ルール予約スキップ除外・スキップ解除復活）、オンエア（放送波フィルタ・番組詳細ポップアップ・キーワード抽出ルール作成遷移・ライブ配信設定モーダル）の網羅的 E2E シナリオを配備（計49件全件 100% PASS） | [テスト仕様書](dev/testing.md#3-playwright-e2e-テスト基盤)、[画面変更仕様書](dev/epgdeck_change_spec.md#34-予約一覧-reserves) |
