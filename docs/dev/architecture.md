...
- **実録画時間（duration）の保存原則**:
  - `recorded.duration` は番組表の予定枠時間ではなく、「チューナー受信開始〜終了の実測ミリ秒」を保存します。これにより、途中終了やドロップ、放送時間延長・短縮時でも実際のファイル再生時間と完全一致します。
- **データのライフサイクルとキャッシュ戦略 (オンデマンド vs 永続化)**:
  - **字幕 (WebVTT)**: 動画再生時のみ単発で要求されるため、ディスク上に `.vtt` を永続化保存せず、FFmpeg pipe ＋ LRU インメモリキャッシュ（ファイルレス）でオンデマンド配信します。
  - **サムネイル (シャーディング & 永続化)**: 録画一覧等で高頻度アクセスされるためディスクへ永続化（WebP/JPEG）して静的配信します。15,000件超の大規模アーカイブ運用に耐えるため、`recordedId % 100` による2桁サブディレクトリ（`00/`〜`99/`）への分散配置を採用。DB に相対パスを保持させることで既存のルート直下ファイルともハイブリッド共存できます。
  - **ドロップログ (不要ファイル自動削除 & DB履歴永続保持)**: ドロップ・エラー0件の正常録画時はログ実ファイルを自動削除（`deleteOnNoDrop: true`）し、ディレクトリ肥大化を防止。実ファイル不在時でも DB 上のエラー・ドロップ数履歴は一切削除せず永続保持し、クリーンアップ走査も `Set` による O(1) 走査で高速化されています。
