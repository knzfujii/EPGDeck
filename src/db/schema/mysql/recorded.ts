...
export const recorded = mysqlTable(
    'recorded',
    {
        id: int('id').autoincrement().primaryKey(),
        reserveId: bigint('reserveId', { mode: 'number' }),
        ruleId: int('ruleId'),
    ...
    table => [
        index('idx_recorded_channel_start').on(table.channelId, table.startAt),
        index('idx_recorded_start_end').on(table.startAt, table.endAt),
        index('idx_recorded_rule').on(table.ruleId),
    ],
);
